...
VOID FuckKeBugCheckEx()
{
	...
	myfun = (ULONGLONG)HookNtTerminateProcess;
...
VOID HookSSDT()
{
	...
	PULONG ServiceTableBase = NULL;
	//get old address
	DbgPrint("NtTerminateProcess index: %ld\n", index);
	OldNtTerminateProcess = (NTTERMINATEPROCESS)GetSSDTFuncCurAddrByIndex(index);
	DbgPrint("Old_NtTerminateProcess: %llx\n", (ULONGLONG)OldNtTerminateProcess);
	//show new address
	ServiceTableBase = (PULONG)KeServiceDescriptorTable->ServiceTableBase;
	//get offset value
	...
	DbgPrint("New_NtTerminateProcess: %llx\n", GetSSDTFuncCurAddrByIndex(index));
}
