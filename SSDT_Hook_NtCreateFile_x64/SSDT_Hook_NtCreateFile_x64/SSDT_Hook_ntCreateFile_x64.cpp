...
VOID HookSSDT()
{
	...
	PULONG ServiceTableBase = NULL;
	//get old address
	DbgPrint("NtCreateFile index: %ld", index);
	NtCreateFile = (NTCREATEFILE)GetSSDTFuncCurAddrByIndex(index);
	DbgPrint("Old_NtCreateFile: %llx", (ULONGLONG)NtCreateFile);
	//show new address
	ServiceTableBase = (PULONG)KeServiceDescriptorTable->ServiceTableBase;
	//get offset value
	...
	DbgPrint("New_NtCreateFile: %llx", GetSSDTFuncCurAddrByIndex(index));
}
